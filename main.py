...
moderators = ["iMooseMoo","GDARKYO ","___Deborah____"]
...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")  
        await self.highrise.walk_to(Position(19.5 , 5.75 , 7.0 , "FrontLeft"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala") 
        if user.username in moderators:
         await self.highrise.chat(f"🚫{user.username} Moderador(a) da sala\n[Entrou na sala]")
         
        await self.highrise.chat(f"Seja bem vindo(a) ao Eu já/Eu nunca 🔞🔥\n\nse prescisar de ajuda diga !help")
